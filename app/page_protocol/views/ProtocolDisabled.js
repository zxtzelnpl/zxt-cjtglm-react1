...
class ProtocolDisabled extends React.Component {
  ...
  render() {
    const score = this.props.score.split('a');
    let sum = 0;
    let type;
    ...
    const questions_html = questions.map((question, question_index) => {
      const choice_html = question.choices.map((choice, choice_index) => {
        const user_choice = parseInt(score[question_index]);
        const value = choice_index + 1;
        if (user_choice === value) {
          return (
            <label key={choice_index}><input type="radio" name={question.key} value={value} checked="checked"
                                             disabled="disabled"/>{choice}</label>
          );
        }

          return (
            <label key={choice_index}><input type="radio" name={question.key} value={value}
                                             disabled="disabled"/>{choice}</label>
          );
      });
      return (
