...
class ScrollText extends React.Component {
  ...
  render() {
    ...
    let textLinkHtml = (<span
      key="last">{linkData.stock.time.slice(5)}《选股牛人-<b>{linkData.name}</b>》推出<b>{linkData.stock.name}</b>{linkData.stock.word}<b>{linkData.stock.rise}</b></span>);
